...
def affine_batch_relu_forward(x, w, b, gamma, beta, bn_param):
    ...
    # Aplly activation (relu) layer.
    out_relu, relu_cache = relu_forward(out_bn)

    cache = (fc_cache, bn_cache, relu_cache)
    return out_relu, cache

def affine_batch_relo_backward(dout, cache):
    fc_cache, bn_cache, relu_cache = cache

    da = relu_backward(dout, relu_cache)

    dbn = batchnorm_backward(da, bn_cache)
    dx, dgamma, dbeta = dbn
...
def affine_layernorm_relu_forward(x, w, b, gamma, beta, bn_param):
    ...
    out_bn, bn_cache = layernorm_forward(out_fc, gamma, beta, bn_param)
...
def affine_layernorm_relu_backward(dout, cache):
    ...
    dbn = layernorm_backward(da, bn_cache)
    dx, dgamma, dbeta = dbn
