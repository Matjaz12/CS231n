...
def discriminator(seed=None):
    ...
    model = nn.Sequential(
        Flatten(), # flatten the input image, s.t we can use the fc layers
        nn.Linear(in_features=784, out_features=256),
        nn.LeakyReLU(negative_slope=0.01),
        nn.Linear(in_features=256, out_features=256),        
        nn.LeakyReLU(negative_slope=0.01),
        nn.Linear(in_features=256, out_features=1) 
        # single output (prob. of sample being real)
    )
...
def generator(noise_dim=NOISE_DIM, seed=None):
    ...
    model = nn.Sequential(
        ...
        nn.Tanh() # to clip the image in range [-1, 1]
    )
...
def discriminator_loss(logits_real, logits_fake):
    ...
    # Fetch labels
    labels_real = torch.ones(logits_real.shape).type(dtype)  # real ... 1
    labels_fake = torch.zeros(logits_fake.shape).type(dtype) # fake ... 0
    
    # Compute loss using BCE(logits, labels)
...
def get_optimizer(model):
    ...
    # beta1 ... decay rate of the moving average of the gradient
    # beta2 ... decay rate of the moving average of the squared gradient
...
def ls_generator_loss(scores_fake):
    ...
    loss = 0.5 * (scores_fake - 1.0).pow(2).mean()
...
def build_dc_generator(noise_dim=NOISE_DIM):
    ...
    model = nn.Sequential(
        ...
        # We use ConvTranspose2d to increase the spatial resolution of the activation map
        nn.ConvTranspose2d(in_channels=128, out_channels=64, kernel_size=(4,4), stride=(2,2), padding=1),
        nn.ReLU(),
        nn.BatchNorm2d(num_features=64),
        nn.ConvTranspose2d(in_channels=64, out_channels=1, kernel_size=(4,4), stride=(2,2), padding=1),
        nn.Tanh(),
        Flatten()
    )
