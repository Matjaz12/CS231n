...
def content_loss(content_weight, content_current, content_target):
    ...
    # Compute the delta between the current content and the content target
...
def gram_matrix(features, normalize=True):
    ...
    # Flatten the width and the height dimensions of feature map
    fetures_r = features.view(features.shape[0], features.shape[1], -1)
    
    # Compute the Gram matrix
    fetures_r_T = fetures_r.permute(0,2,1) # transpose along width and height
    gram = torch.bmm(fetures_r, fetures_r_T)

    gram = gram / (C * H * W) if normalize else gram
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for idx, layer in enumerate(style_layers):
        style_curr = gram_matrix(feats[layer])
        style_target = style_targets[idx]
        style_weight = style_weights[idx]

        # Compute style loss for this layer
        loss += content_loss(style_weight, style_curr, style_target)
...
def tv_loss(img, tv_weight):
    ...
    # Penalize variation in the pixel values between pixels that are next to
    # each other (horizontally and vertically)
    loss_vertical = tv_weight * torch.sum((img[:,:,0:-1,:] - img[:,:,1: ,:]) ** 2)
    loss_horziontal = tv_weight * torch.sum((img[:,:,:,0:-1] - img[:,:,:,1: ]) ** 2)

    loss = loss_vertical + loss_horziontal 
